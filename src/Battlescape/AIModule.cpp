...
void AIModule::setupAmbush()
{
	...
	if (selectClosestKnownEnemy())
	{
		...
		for (const auto* node : *_save->getNodes())
		{
			...
			if(((Options::battleRealisticAccuracy && !_save->getTileEngine()->canTargetUnitRCAS(&origin, tile, &target, _aggroTarget, false, _unit)) 
		...
		if (bestScore > 0)
		{
			...
			while (tries > 0)
			{
				...
				if(((Options::battleRealisticAccuracy && _save->getTileEngine()->canTargetUnitRCAS(&origin, tile, &target, _unit, false, _aggroTarget)) 
...
int AIModule::getSpottingUnits(const Position& pos) const
{
	...
	for (auto* bu : *_save->getUnits())
	{
		if (validTarget(bu, false, false))
		{
			...
			if (checking)
			{
				if(((Options::battleRealisticAccuracy && _save->getTileEngine()->canTargetUnitRCAS(&originVoxel, _save->getTile(pos), &targetVoxel, bu, false, _unit)) 
			...
			{
				if(((Options::battleRealisticAccuracy && _save->getTileEngine()->canTargetUnitRCAS(&originVoxel, _save->getTile(pos), &targetVoxel, bu, false)) 
			...
			_save->getTileEngine()->visible(_unit, bu->getTile()))
		{
			...
			if (dist < _closestDist)
			{
				...
				if (_rifle || !_melee)
				{
					...
					valid = (Options::battleRealisticAccuracy && _save->getTileEngine()->canTargetUnitRCAS(&origin, bu->getTile(), &target, _unit, false))
...
int AIModule::scoreFiringMode(BattleAction *action, BattleUnit *target, bool checkLOF)
{
	...
	if (checkLOF)
	{
		...
		{
			if(((Options::battleRealisticAccuracy && !_save->getTileEngine()->canTargetUnitRCAS(&origin, target->getTile(), &targetPosition, _unit, false, target)) 
...
bool AIModule::findFirePoint()
{
	...
	for (const auto& randomPosition : randomTileSearch)
	{
		...
		if(((Options::battleRealisticAccuracy && _save->getTileEngine()->canTargetUnitRCAS(&origin, _aggroTarget->getTile(), &target, _unit, false)) 

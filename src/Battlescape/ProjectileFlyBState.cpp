...
void ProjectileFlyBState::init()
{
	...
	else if (!_action.weapon->getArcingShot(_action.type))
	{
			...
			targetTile->getUnit()->getVisible()))
		{
			...
			else
			{
				...
				if(Options::battleRealisticAccuracy)
					foundLoF = _parent->getTileEngine()->canTargetUnitRCAS(&originVoxel, targetTile, &_targetVoxel, _unit, isPlayer);
				else
		...
		else if (Options::battleRealisticAccuracy) // RCAS
		{
...
bool ProjectileFlyBState::createNewProjectile()
{
	...
	if (_action.type == BA_THROW)
	{
		...
		if (_projectileImpact == V_FLOOR || _projectileImpact == V_UNIT || _projectileImpact == V_OBJECT || _projectileImpact == V_WESTWALL || _projectileImpact == V_NORTHWALL || _projectileImpact == V_EMPTY) // RCAS		
		{

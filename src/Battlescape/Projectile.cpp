...
int Projectile::calculateTrajectory(double accuracy, const Position& originVoxel, bool excludeUnit)
{
	...
	if(Options::battleRealisticAccuracy)
		applyAccuracyRCAS(originVoxel, &_targetVoxel, accuracy, false, extendLine);	
	else
...
int Projectile::calculateThrow(double accuracy)
{
	...
	while (test == V_OUTOFBOUNDS && tries < 100)
	{
		...
		{			
			...
			if(Options::battleRealisticAccuracy)
				applyAccuracyRCAS(originVoxel, &deltas, accuracy, true, false); 
			else
		...
		{
			if(Options::battleRealisticAccuracy) // NHR: Bug or RCAS?
			{				
				applyAccuracyRCAS(originVoxel, &targetVoxel, accuracy, true, false); //arcing shot deviation				
 ...
 * @param extendLine should this line get extended to maximum distance?
 */
void Projectile::applyAccuracyRCAS(Position origin, Position *target, double accuracy, bool keepRange, bool extendLine)
{

...
RuleBaseFacility::RuleBaseFacility(const std::string &type, int listOrder) :
	...
	_prisonType(0), _hangarType(-1), _rightClickActionType(0), _verticalLevels(), _removalTime(0), _canBeBuiltOver(false), _upgradeOnly(false), _destroyedFacility(0)
{
}
...
void RuleBaseFacility::load(const YAML::YamlNodeReader& node, Mod *mod)
{
	...
	reader.tryRead("craftSlots", _craftSlots);	
